...
class ListStory extends LitElement {
  ...
  async _initialData() {
    this.isLoading = true;
    try {
      await this._simulateLoading(1000);

      const newFetchRecords = await EndpointStory.getAll();
      const response = newFetchRecords.data.listStory;
    ...
    this.isLoading = false;
    this._removeAlertAfterTimeout(4000);
  }

  async _simulateLoading(milliseconds) {
    return new Promise((resolve) => setTimeout(resolve, milliseconds));
  }

  _removeAlertAfterTimeout(timeout) {
    setTimeout(() => {
  ...
  render() {
    return this.isLoading ? this.renderLoading() : this.renderStories();
  }
}
